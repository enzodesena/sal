...
class Matrix {
 public:
    ...
    if (num_rows_ > 0) {
      ...
      for (Int i = 1; i < num_rows_; ++i) {
        ...
        if ((Int)vectors[i].size() != num_columns_) {
          ASSERT_WITH_MESSAGE(
              false, "One or more rows do not have the same number of columns");
        }
      }
      data_ = vectors;
    } else {
  ...
  /** Same as Save(...), but before saving converts decimal numbers (assumed to
   be in the range -1, 1) to integers using the formula:
  ...
  void SaveIntegerFormat(std::string file_name, dsp::Int bit_precision = 24) {
    ...
    for (Int i = 0; i < num_rows_; ++i) {
      for (Int j = 0; j < num_columns_; ++j) {
        output_file << static_cast<int>(data_.at(i).at(j) *
                                        ((1 << (bit_precision - 1)) - 1))
                    << " ";
      }
      output_file << std::endl;
...
/** Carries out the mulitplication between  matrix_a * vector, where vector is a
 column vector. The output is also a column vector. */
template <class T>
...
/** Carries out the mulitplication between vector * matrix_a, where vector is a
 row vector. The output is also a row vector. */
template <class T>
