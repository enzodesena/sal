...
public:
  ...
  Buffer(const size_t num_channels, const size_t num_samples) :  temporary_vector_(std::vector<Sample>(num_samples, 0.0)) {
  ...
  static void AddBuffers(const Buffer& input_buffer_a, const Buffer& input_buffer_b, Buffer& output_buffer) noexcept {
    ...
    for (size_t chan_id = 0; chan_id<input_buffer_a.num_channels(); ++chan_id) {
      mcl::Add(input_buffer_a.GetReadView(chan_id), input_buffer_b.GetReadView(chan_id),
...
//
//    mcl::Add(samples,
//             std::span<Sample>(data_[channel_id].begin() + from_sample_id, samples.size()),
//             std::span<Sample>(data_[channel_id].begin() + from_sample_id, samples.size()));
//  }
//
