...
class DelayFilter : public mcl::Filter {
 public:
  /**
   ...
   maximum latency has to be given to allocate the maximum amount of memory of
   the circular memory.
   */
  ...
  inline const Sample& ReadAt(const Int delay_tap) const noexcept {
    ...
    return (read_index >= start_) ? *read_index
  ...
  inline Sample FractionalReadAt(
    ...
    Time sanitised_delay_tap =
        std::min(fractional_delay_tap, (Time)max_latency_);
