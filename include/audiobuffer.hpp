...
public:
  BFormatBuffer(
    ...
    const size_t num_samples)
    : Buffer<T>(GetNumChannels(max_degree)
    , num_samples)
  {
  }
  ...
  {
    Buffer<T>::AddSamples(
      GetChannelId(degree, order), from_sample_id, num_samples, samples);
  }
