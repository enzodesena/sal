...
class FreeFieldSim {
 public:
  FreeFieldSim(std::vector<Microphone*> microphones,
               std::vector<Source*> sources, const Time sampling_frequency,
               const Length sound_speed);

  FreeFieldSim(Microphone* microphones, std::vector<Source*> sources,
               const Time sampling_frequency, const Length sound_speed);

  FreeFieldSim(std::vector<Microphone*> microphones, Source* sources,
               const Time sampling_frequency, const Length sound_speed);

  FreeFieldSim(Microphone* microphones, Source* sources,
               const Time sampling_frequency, const Length sound_speed);

  void Init(std::vector<Microphone*> microphones, std::vector<Source*> sources,
            const Time sampling_frequency, const Length sound_speed);

  void ProcessBlock(
      const std::vector<std::unique_ptr<MonoBuffer>>& input_buffers,
 ...
 private:
  ...
  static std::vector<Length> AllDistances(
