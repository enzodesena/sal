...
bool Microphone::Test() {
  ...
  TrigMic mic_i(Point(0.0, 0.0, 0.0), dsp::AxAng2Quat(0, 0, 1, PI / 4.0),
                coefficients);
  ...
  TrigMic mic_e(Point(1.0, 0.0, 0.0), dsp::AxAng2Quat(0, 0, 1, PI / 2.0),
                coefficients);
  ASSERT(IsEqual(
      mic_e.RecordPlaneWave(0.5, Point(0.0, 1.0, 0.0)),
      0.5 * (0.5 - 1.0 * cos(PI / 4.0) - 0.5 * pow(cos(PI / 4.0), 2.0))));

  TrigMic mic_f(Point(2.0, 0.0, 0.0), dsp::AxAng2Quat(0, 0, 1, PI / 2.0),
                coefficients);
  ...
  TrigMic mic_g(Point(0.0, 0.0, 0.0), dsp::Quaternion::Identity(),
                coefficients);
  ASSERT(IsEqual(mic_g.RecordPlaneWave(0.5, Point(1.0, 0.0, 0.0)),
                 0.5 * (0.5 - 1.0 - 0.5)));

  TrigMic mic_h(Point(0.0, 0.0, 0.0), dsp::AxAng2Quat(0, 0, 1, PI / 5.0),
                coefficients);
  ASSERT(
      IsEqual(mic_h.RecordPlaneWave(0.5, Point(2.0, 2.0 * tan(PI / 5.0), 0.0)),
              0.5 * (0.5 - 1.0 - 0.5)));

  TrigMic mic_p(Point(2.0, 1.2, 0.5), dsp::AxAng2Quat(0, 0, 1, 0),
                coefficients);
  Sample sample =
