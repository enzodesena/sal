...
bool MicrophoneArrayTest() {
  ...
  CircularArray<TrigMic> microphone_array_a(
      Point(0.0, 0.0, 1.5), mcl::Quaternion::Identity(), mic_prototype,
  ...
  ASSERT(mcl::IsEqual(microphone_array_a.GetMicrophoneRef(0).orientation(),
                      mcl::AxAng2Quat(0, 0, 1, 0)));
  ASSERT(mcl::IsEqual(microphone_array_a.GetMicrophoneRef(1).orientation(),
  ...
  CircularArray<TrigMic> microphone_array_b(
      Point(0.0, 0.0, 0.0), mcl::AxAng2Quat(0, 0, 1, PI / 2.0), mic_prototype,
      array_radius, UniformAngles<Angle>(num_microphones, 0));
  ...
  CircularArray<TrigMic> microphone_array_c(
      Point(0.0, 1.0, -1.0), mcl::AxAng2Quat(0, 0, 1, PI / 2.0), mic_prototype,
      array_radius, UniformAngles<Angle>(num_microphones, 0));
  ...
  StereoMic<TrigMic> stereo_mic(Point(0.2, 0.0, 1.5),
                                mcl::Quaternion::Identity(), mic_prototype, 1.0,
                                PI / 4.0, +PI / 8.0);

  ASSERT(mcl::IsEqual(stereo_mic.GetMicrophoneRef(0).position(),
                      Point(0.2 + 1.0 * cos(0), 1.0 * sin(0), 1.5)));
  ASSERT(
      mcl::IsEqual(stereo_mic.GetMicrophoneRef(1).position(),
