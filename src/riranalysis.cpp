  ...
  mcl::Vector<sal::Sample> rir)
{
  ...
  return mcl::Multiply<sal::Sample>
    (mcl::Log10(mcl::Flip(mcl::Multiply<sal::Sample>(cs, 1.0 / cs[cs.size() - 1]))), 10.0);
}

sal::Time RirAnalysis::Rt60(
  mcl::Vector<sal::Sample> rir,
  sal::Time sampling_frequency) { return Rs60(rir) / sampling_frequency; }

sal::Time RirAnalysis::Rs60(
  mcl::Vector<sal::Sample> rir) { return Rs(rir, -60.0); }
...
sal::Time RirAnalysis::Rs60FromEdc(
  mcl::Vector<sal::Sample> edc) { return RsFromEdc(edc, -60.0); }
  ...
  sal::Sample decay_value)
{
  ...
  for (sal::Int i = 0; i < N; ++i) { if (edc[i] < decay_value) { return i; } }
  return N;
}
} // namespace sal
