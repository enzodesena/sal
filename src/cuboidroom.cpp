...
bool CuboidRoom::IsPointInRoom(const Point& point,
  ...
  return std::isgreaterequal(point.x(), origin_position_.x() + wall_distance) &&
         ...
         std::islessequal(point.x(), origin_position_.x() + dimensions_.x() -
                                         wall_distance) &&
         std::islessequal(point.y(), origin_position_.y() + dimensions_.y() -
                                         wall_distance) &&
         std::islessequal(
             point.z(), origin_position_.z() + dimensions_.z() - wall_distance);
}
...
std::vector<Point> CuboidRoom::CalculateBoundaryPoints(
  ...
  reflection_points[0] =
      ReflectionPoint(kX1, shifted_source_point, shifted_mic_point);
  reflection_points[1] =
      ReflectionPoint(kX2, shifted_source_point, shifted_mic_point);
  reflection_points[2] =
      ReflectionPoint(kY1, shifted_source_point, shifted_mic_point);
  reflection_points[3] =
      ReflectionPoint(kY2, shifted_source_point, shifted_mic_point);
  reflection_points[4] =
      ReflectionPoint(kZ1, shifted_source_point, shifted_mic_point);
  reflection_points[5] =
  ...
  if (boundary_set_type_ == kFirstAndSecondOrder) {
    ...
    Point b_kY2 = IntersectionPoint(
        kY2, dimensions_, shifted_mic_point,
        ImageSourcePosition(shifted_source_point, 1, 1, 0, 1, 1, 0));
    ...
    Point c_kX1 = IntersectionPoint(
        kX1, dimensions_, shifted_mic_point,
...
std::vector<mcl::IirFilter> CuboidRoom::GetBoundaryFilters(
  ...
  if (boundary_set_type_ == kFirstAndSecondOrder) {
    Point a_kX2 =
        IntersectionPoint(kX2, dimensions_, mic_point,
    ...
    Point b_kX2 =
        IntersectionPoint(kX2, mic_point, dimensions_,
    ...
    Point d_kX1 =
        IntersectionPoint(kX1, mic_point, dimensions_,
...
std::string CuboidRoom::ShapeDescription() const noexcept {
  return "The room is rectangular with " + ToString(origin_position_.x()) +
         "<x<" + ToString(origin_position_.x() + dimensions_.x()) + " " +
