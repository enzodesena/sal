      ...
      (UInt)ceil(room->max_distance() * sampling_frequency)))
{
  ...
  for (Int i = 0; i < num_elements_; ++i)
  {
    ...
    (
      ...
      (
        ...
        spatial_sampling_period,
        specific_acoustic_impedance));
    weights_mic_previous_.push_back
    (
  ...
  if (true)
  {
    ...
    std::cout << "Spatial sampling period: " << spatial_sampling_period << std::
      endl;
...
Sample TdBem::CalculateDrDn(
  ...
  Point normal_y)
{
  return mcl::DotProduct
  (
    mcl::Subtract(point_y, point_x),
  ...
  for (Int sample_id = 0; sample_id < input_buffer.num_samples(); ++sample_id)
  {
    ...
    for (Int i = 0; i < num_elements_; ++i)
    {
      ...
      for (Int j = 0; j < num_elements_; ++j)
      {
        ...
        pressure += (pressures_[j].FractionalReadAt(delay) * weights_current_[i]
            [j] +
    ...
    for (Int i = 0; i < num_elements_; ++i)
    {
      ...
      microphone_->AddPlaneWave(
        MonoBuffer::Unary(pressure), points_[i], i, mono_output_buffer);
    }
