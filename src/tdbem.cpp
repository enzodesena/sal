  ...
  Buffer& output_buffer)
{
  ...
  for (Int sample_id = 0; sample_id < input_buffer.num_samples(); ++sample_id)
  {
    ...
    for (Int i = 0; i < num_elements_; ++i)
    {
      ...
      if (log_) { boundary_pressure.SetElement(i, k, pressure); }
    }
    ...
    (MonoBuffer::Unary
     (source_delay_line_.FractionalReadAt(los_delay) / (4.0 * PI * distance_los_)),
     source_->position(),
     num_elements_,
     mono_output_buffer);
    ...
    for (Int i = 0; i < num_elements_; ++i) { pressures_[i].Tick(); }
    k++;
  }
