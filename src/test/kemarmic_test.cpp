...
bool KemarMic::Test() {
  ...
  ASSERT(IsEqual(buffer_i.GetLeftReadPointer(),
                 buffer_i.GetRightReadPointer(),
                 impulse_response_length));
  
  ASSERT(IsEqual(cmp_imp_front_left, buffer_i.GetLeftReadPointer()));
  ...
  ASSERT(mcl::IsEqual(cmp_imp_front_left, buffer_o.GetLeftReadPointer()));
  ...
  ASSERT(IsEqual(buffer_m.GetLeftReadPointer(), cmp_imp_up_left));
  ...
  ASSERT(mcl::IsEqual(buffer_p.GetLeftReadPointer(), cmp_imp_right_left));
  ASSERT(mcl::IsEqual(buffer_p.GetRightReadPointer(), cmp_imp_right_right));
  ...
  ASSERT(mcl::IsEqual(cmp_imp_right_left,
                      buffer_pa.GetLeftReadPointer()));
  ASSERT(mcl::IsEqual(cmp_imp_right_right,
                      buffer_pa.GetRightReadPointer()));
  ...
  ASSERT(mcl::IsEqual(buffer_r.GetLeftReadPointer(), cmp_imp_left_left));
  ASSERT(mcl::IsEqual(buffer_r.GetRightReadPointer(), cmp_imp_left_right));
  ...
  ASSERT(mcl::IsEqual(buffer_t.GetLeftReadPointer(), cmp_imp_back));
  ASSERT(mcl::IsEqual(buffer_t.GetRightReadPointer(), cmp_imp_back));
  
  // Testing reset
  buffer_t.Reset();
  
  mic_t.AddPlaneWave(MonoBuffer::Unary(1.0), Point(0.0,0.0,-1.0), buffer_t);
  ASSERT(! IsEqual(buffer_t.GetLeftReadPointer()[0], 0.0, 1.0E-10));
  ASSERT(! IsEqual(buffer_t.GetRightReadPointer()[0], 0.0, 1.0E-10));
  ...
  mic_t.Reset();
  mic_t.AddPlaneWave(MonoBuffer::Unary(0.0), Point(0.0,0.0,-1.0), buffer_t);
  ASSERT(IsEqual(buffer_t.GetLeftReadPointer()[0], 0.0));
  ASSERT(IsEqual(buffer_t.GetRightReadPointer()[0], 0.0));
  ...
  Signal output_u_left(buffer_u.GetLeftReadPointer(),
                       buffer_u.GetLeftReadPointer()+impulse_response_length);
  Signal output_u_right(buffer_u.GetRightReadPointer(),
                        buffer_u.GetRightReadPointer()+impulse_response_length);
  ...
  output_u_left = Signal(buffer_u.GetLeftReadPointer(),
                         buffer_u.GetLeftReadPointer()+impulse_response_length);
  output_u_right = Signal(buffer_u.GetRightReadPointer(),
                          buffer_u.GetRightReadPointer()+impulse_response_length);
  ...
  Signal output_u_b_left(buffer_u.GetLeftReadPointer(),
                         buffer_u.GetLeftReadPointer()+impulse_response_length);
  Signal output_u_b_right(buffer_u.GetRightReadPointer(),
                          buffer_u.GetRightReadPointer()+impulse_response_length);
  ...
  ASSERT(IsEqual(buffer_diffuse.GetLeftReadPointer(), mcl::Multiply(cmp_diffuse_up_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_diffuse.GetRightReadPointer(), mcl::Multiply(cmp_diffuse_up_left_ear, normalising_value)));
  ...
  mic_diffuse.AddPlaneWave(impulse, Point(cos(-azimuth)*cos(elevation), sin(-azimuth)*cos(elevation), sin(elevation)), buffer_diffuse);
  ...
  ASSERT(IsEqual(buffer_diffuse.GetLeftReadPointer(), mcl::Multiply(cmp_diffuse_elevation_40_azimuth_77_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_diffuse.GetRightReadPointer(), mcl::Multiply(cmp_diffuse_elevation_40_azimuth_77_right_ear, normalising_value)));
  
  mic_diffuse.Reset();
  buffer_diffuse.Reset();
  // Check for the other side (hence `azimuth` is positive here)
  mic_diffuse.AddPlaneWave(impulse, Point(cos(azimuth)*cos(elevation), sin(azimuth)*cos(elevation), sin(elevation)), buffer_diffuse);
  ASSERT(IsEqual(buffer_diffuse.GetLeftReadPointer(), mcl::Multiply(cmp_diffuse_elevation_40_azimuth_77_right_ear, normalising_value)));
  ASSERT(IsEqual(buffer_diffuse.GetRightReadPointer(), mcl::Multiply(cmp_diffuse_elevation_40_azimuth_77_left_ear, normalising_value)));
  ...
  ASSERT(IsEqual(buffer_compact.GetLeftReadPointer(), mcl::Multiply(cmp_compact_up_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_compact.GetRightReadPointer(), mcl::Multiply(cmp_compact_up_left_ear, normalising_value)));
  ...
  // The minus for azimuth is because for our reference system, a positive azimuth angle is to the left of the person, while for kemar is on the right
  mic_compact.AddPlaneWave(impulse, Point(cos(-azimuth)*cos(elevation), sin(-azimuth)*cos(elevation), sin(elevation)), buffer_compact);
  ...
  ASSERT(IsEqual(buffer_compact.GetLeftReadPointer(), mcl::Multiply(cmp_compact_elevation_40_azimuth_77_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_compact.GetRightReadPointer(), mcl::Multiply(cmp_compact_elevation_40_azimuth_77_right_ear, normalising_value)));
  
  mic_compact.Reset();
  buffer_compact.Reset();
  // Check for the other side (hence `azimuth` is positive here)
  mic_compact.AddPlaneWave(impulse, Point(cos(azimuth)*cos(elevation), sin(azimuth)*cos(elevation), sin(elevation)), buffer_compact);
  ASSERT(IsEqual(buffer_compact.GetLeftReadPointer(), mcl::Multiply(cmp_compact_elevation_40_azimuth_77_right_ear, normalising_value)));
  ASSERT(IsEqual(buffer_compact.GetRightReadPointer(), mcl::Multiply(cmp_compact_elevation_40_azimuth_77_left_ear, normalising_value)));
  ...
  KemarMic mic_full(Point(0.0,0.0,0.0), mcl::Quaternion::Identity(), kFullDataset);
  StereoBuffer buffer_full(long_impulse_response_length);

  mic_full.AddPlaneWave(long_impulse, Point(0.0,0.0,1.0), buffer_full);

  Signal cmp_full_up_left_ear = {-1,-1,0,0,1,1,0,-2,-1,2,4,5,3,1,-2,-5,-4,-4,0,-1,-2,-4,-6,3,8,3,4,-11,16,-25,39,-48,73,-30,1546,3893,5588,373,-10031,-5025,6528,6989,2867,2455,2630,1273,457,-2740,-6547,-7058,-5361,-4545,-3695,-1328,539,2002,3401,4573,4285,3232,2445,1509,78,-1299,-1421,-1253,-1556,-2011,-2142,-2278,-2110,-1193,-145,351,685,1005,1001,1248,1689,1569,893,184,-204,-373,-357,-522,-1162,-1732,-1593,-991,-466,62,318,402,557,666,491,91,-149,-129,-179,-474,-592,-286,47,42,-69,-106,-156,-235,-149,64,228,250,75,-177,-197,55,252,113,-295,-574,-466,-96,200,197,53,36,147,290,343,257,39,-178,-251,-209,-125,-64,-62,-91,-84,-15,97,199,207,114,13,-34,5,92,124,51,-35,-61,-35,-3,25,24,-9,0,53,102,138,149,112,69,56,50,17,-14,-4,40,99,137,161,169,109,67,25,-60,-80,-46,-4,57,78,32,19,57,97,125,106,64,46,33,17,19,24,-14,-36,-22,-34,-10,68,78,22,4,27,58,100,139,129,85,74,89,77,62,58,38,22,25,19,0,0,18,39,40,45,69,83,78,49,21,33,64,62,35,19,14,8,15,50,99,110,76,4,-47,-48,-43,72,183,165,93,74,56,-41,-77,-77,-80,-47,22,61,43,15,-39,-108,-148,-93,14,98,146,167,210,237,309,389,251,24,-75,-99,-133,-114,-95,-165,-244,-229,-155,-147,-208,-188,-150,-192,-49,235,406,509,498,218,-52,-126,-157,-252,-365,-428,-350,-143,23,76,123,158,169,258,309,240,119,21,-41,-67,-73,-126,-194,-200,-146,-98,-50,8,50,77,77,113,181,202,130,64,48,14,8,2,-63,-137,-114,86,142,-246,-455,-324,-207,3,217,239,172,191,224,136,-109,-375,-392,-99,174,159,82,154,256,116,-66,-141,-247,-267,-247,-292,-330,-199,-2,37,-31,21,131,196,283,363,378,265,40,-186,-328,-343,-413,-464,-286,-50,120,177,234,266,190,156,123,49,29,20,-71,-195,-145,-67,-130,-152,-71,75,108,71,87,86,69,43,-32,-134,-132,-76,-105,-82,-80,-65,-21,56,95,37,35,-13,74,175,97,-19,-31,-42,-127,-245,-389,-304,-120,-49,0,149,243,235,206,73,-2,19,44,-23,-101,-60,58,43,-93,-200,-234,-179,-104,-44,17,112,132,76,26,49,86,36,-71,-119,-66,-61,-121,-145,-92,8,96,47,3,125,151,13,-104,-182,-193,-77,78,127,157,183,49,-59,-22,16,22,7,-19,-8,29,-4,-58,-68,-9,8,-53,-34,-22,-28,15,27,-29,-98,-57};

  ASSERT(IsEqual(buffer_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_up_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_full.GetRightReadPointer(), mcl::Multiply(cmp_full_up_left_ear, normalising_value)));
  ...
  buffer_full.Reset();
  // The minus for azimuth is because for our reference system, a positive azimuth angle is to the left of the person, while for kemar is on the right
  mic_full.AddPlaneWave(long_impulse, Point(cos(-azimuth)*cos(elevation), sin(-azimuth)*cos(elevation), sin(elevation)), buffer_full);
  
,2031,1057,-614,-2105,-2126,-1348,-1046,-1284,-1522,-1225,-667,152,853,1115,909,675,517,589,817,796,278,-302,-539,-591,-678,-777,-733,-693,-661,-504,-197,215,641,868,786,484,210,81,135,109,-99,-410,-579,-535,-310,-49,50,1,-140,-181,-11,236,291,162,-54,-180,-132,68,232,218,40,-160,-164,29,201,221,98,-72,-107,41,193,197,96,0,-30,25,138,169,108,8,-32,-1,60,112,102,31,-40,-32,18,59,73,63,30,26,76,92,41,-6,0,45,68,71,59,29,31,57,77,70,53,35,29,48,51,34,53,100,120,101,79,57,26,26,64,66,33,8,-12,-18,13,70,98,67,14,1,28,76,128,136,101,65,56,53,67,120,145,98,38,-7,-119,-230,-189,-37,137,234,262,230,85,-60,-104,-136,-241,-243,-7,266,303,175,100,146,241,198,99,7,-141,-203,-248,-336,-325,-195,-33,39,48,157,308,338,232,105,52,68,89,52,27,-31,-129,-134,-139,-176,-185,-164,-77,9,17,46,177,254,256,244,177,109,49,-35,-142,-202,-155,-105,-87,-83,-62,11,16,32,125,133,164,250,230,168,120,-4,-235,-410,-403,-321,-251,-217,-181,-73,194,534,599,367,257,281,151,-118,-416,-529,-365,-102,-22,-255,-335,-21,312,345,234,178,108,-28,-160,-233,-227,-157,-154,-76,87,191,249,176,45,-94,-230,-349,-316,-144,-45,42,153,186,221,267,213,83,0,-115,-259,-292,-296,-234,-107,52,200,226,226,290,279,84,-132,-145,-96,-103,-144,-227,-192,-32,71,57,41,50,15,-17,18,90,89,26,-29,-39,-17,-50,-100,-112,-103,-127,-193,-161,-48,39,54,22,46,85,115,136,45,-55,-49,-27,-32,-59,-106,-151,-110,-61,-62,-43,3,43,53,36,36,43,-25,-54,-8,-4,-37,-57,-52,-57,-83,-96,-53,35,65,5,-48,-48,-44,-30,23,87,97,30,20,74,41,4,16,-15,-112,-186,-125,-44,-34,-50,-83,-28,53,48,44,30,41,73,18,-24,-52,-43,41,9,-108,-157,-106,10,53,-15,-51,2,76,87,50,29,14,11,22,29,7,-47,-84,-80,-66,-37,-6,-31,-67,-77,-32,22,23,-13,-67,-45,28,55,34};
  
  ASSERT(IsEqual(buffer_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_left_ear, normalising_value)));
  ASSERT(IsEqual(buffer_full.GetRightReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_right_ear, normalising_value)));
  
  mic_full.Reset();
  buffer_full.Reset();
  // Check for the other side (hence `azimuth` is positive here)
  mic_full.AddPlaneWave(long_impulse, Point(cos(azimuth)*cos(elevation), sin(azimuth)*cos(elevation), sin(elevation)), buffer_full);
  ASSERT(IsEqual(buffer_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_right_ear, normalising_value)));
  ASSERT(IsEqual(buffer_full.GetRightReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_left_ear, normalising_value)));
