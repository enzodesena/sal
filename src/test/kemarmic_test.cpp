...
bool KemarMic::Test() {
  ...
  StereoBuffer stream_full(long_impulse_response_length);

  mic_full.AddPlaneWave(long_impulse, Point(0.0,0.0,1.0), stream_full);
  ...
  ASSERT(IsEqual(stream_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_up_left_ear, normalising_value)));
  ASSERT(IsEqual(stream_full.GetRightReadPointer(), mcl::Multiply(cmp_full_up_left_ear, normalising_value)));
  ...
  stream_full.Reset();
  // The minus for azimuth is because for our reference system, a positive azimuth angle is to the left of the person, while for kemar is on the right
  mic_full.AddPlaneWave(long_impulse, Point(cos(-azimuth)*cos(elevation), sin(-azimuth)*cos(elevation), sin(elevation)), stream_full);
  ...
  ASSERT(IsEqual(stream_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_left_ear, normalising_value)));
  ASSERT(IsEqual(stream_full.GetRightReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_right_ear, normalising_value)));
  
  mic_full.Reset();
  stream_full.Reset();
  // Check for the other side (hence `azimuth` is positive here)
  mic_full.AddPlaneWave(long_impulse, Point(cos(azimuth)*cos(elevation), sin(azimuth)*cos(elevation), sin(elevation)), stream_full);
  ASSERT(IsEqual(stream_full.GetLeftReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_right_ear, normalising_value)));
  ASSERT(IsEqual(stream_full.GetRightReadPointer(), mcl::Multiply(cmp_full_elevation_40_azimuth_77_left_ear, normalising_value)));
