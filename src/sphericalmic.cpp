...
SphericalHeadMic::SphericalHeadMic(
    const Point position, const Quaternion orientation, const Angle ears_angle,
    const Length sphere_radius, const Int ir_length,
    const Time sampling_frequency, const Int update_length)
...
Angle SphericalHeadMic::GetTheta(const Point& point, const Angle& ears_angle,
                                 const Ear& ear) {
...
mcl::Complex SphericalHeadMic::Sphere(Length a, Length r, Angle theta, Time f,
  ...
  while (oldratio > threshold || newratio > threshold) {
    // Qr = - (2 * m - 1) * zr * Qr1 + Qr2;
    Complex Qr =
        -(Complex(2.0, 0.0) * ((Complex)m) - Complex(1.0, 0.0)) * zr * Qr1 +
        Qr2;
    // Qa = - (2 * m - 1) * za * Qa1 + Qa2;
    Complex Qa =
        -(Complex(2.0, 0.0) * ((Complex)m) - Complex(1.0, 0.0)) * za * Qa1 +
        Qa2;
    // P = ( (2 * m - 1) * x * P1 - (m - 1) * P2) / m;
    Complex P =
        ((Complex(2.0, 0.0) * ((Complex)m) - Complex(1.0, 0.0)) * x * P1 -
         (((Complex)m) - Complex(1.0, 0.0)) * P2) /
        ((Complex)m);
...
Signal SphericalHeadMic::GetBrir(const Ear ear, const Point& point) noexcept {
  return GenerateImpulseResponse(sphere_radius_,
                                 ...
                                 sound_speed_, alg_threshold_,
                                 impulse_response_length_, sampling_frequency_);
}
