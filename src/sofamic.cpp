...
SofaMic::SofaMic(const Point position, const Quaternion orientation,
    ...
    : BinauralMic(position, orientation, update_length, reference_orientation),
      hrtf_(NULL) {
  int err;
  // TODO: Still need to implement reference_orientation for orientations other
  // than standard

  hrtf_ = mysofa_open(sofa_filename.c_str(), (int)sampling_frequency,
                      &filter_length_, &err);
...
Signal SofaMic::GetBrir(const Ear ear, const Point& point) noexcept {
  ...
  mysofa_getfilter_float(hrtf_, (float)point.x(), (float)point.y(),
                         (float)point.z(), left_impulse_response,
                         right_impulse_response, &left_delay, &right_delay);
  ...
  if (ear == kLeftEar) {
    std::transform(left_impulse_response,
                   left_impulse_response + filter_length_,
                   output_impulse_response.begin(),
                   [](float value) { return static_cast<Sample>(value); });
  } else {
    std::transform(right_impulse_response,
                   right_impulse_response + filter_length_,
                   output_impulse_response.begin(),
                   [](float value) { return static_cast<Sample>(value); });
  }
...
SofaMic::~SofaMic() { mysofa_close(hrtf_); }
