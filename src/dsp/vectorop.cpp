...
#else
  vDSP_vma(input_data_mult.data(), 1, &gain, 0, input_data_add.data(), 1,
           output_data.data(), 1, num_samples);
#endif
#else
  for (size_t i = 0; i < num_samples; ++i) {
...
Real Mean(const std::vector<Real>& input,
  ...
  std::vector<Real> normalised_weights =
      Multiply<Real>(weights, 1.0 / Sum(weights));
...
Real Std(const std::vector<Real>& input) noexcept { return sqrt(Var(input)); }
...
std::vector<std::vector<Real> > Enframe(const std::vector<Real>& input,
  ...
  while ((i + window.size()) <= input.size()) {
    ...
    output.push_back(Multiply(Elements(input, from_sample, to_sample), window));
