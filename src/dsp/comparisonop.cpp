...
bool IsNan(Real num) { return isnan(num); }
...
bool IsInf(Real num) { return std::isinf(num); }
...
bool None(std::vector<bool> input) { return All(Not(input)); }
...
bool AreAllSmallerOrEqual(const std::vector<Real>& vector_a,
  ...
  for (Int i = 0; i < (Int)(Int)vector_a.size(); ++i) {
    if (!IsSmallerOrEqual(vector_a[i], vector_b[i])) {
      return false;
    }
  }
