...
PaWrapper::PaWrapper(Time sampling_frequency, Int out_dev_num,
  ...
  Pa_OpenStream(&stream_, NULL, &output_parameters, sampling_frequency,
                paFramesPerBufferUnspecified,
                paNoFlag,  // flags that can be used to define dither, clip
                           // settings and more
                NULL,   // your callback function
                NULL);  // data to be passed to callback. In C++, it is
                        // frequently (void *)this
}
...
PaError PaWrapper::WriteStream(const Buffer& output_buffer) {
  ...
  for (Int i = 0; i < num_samples; ++i) {
    for (Int j = 0; j < max_num_channels; ++j) {
      float sample;
      if (channel_ids_[j] == -1 || i >= output_buffer.num_samples()) {
        sample = 0.0;
      } else {
