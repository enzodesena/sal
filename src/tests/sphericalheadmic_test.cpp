...
namespace sat {
  ...
  bool SphericalHeadMic::Test() {
    ...
    std::vector<Signal> output_a = stream_a->Pull(6);
    ...
    assert(IsEqual(output_a[0], output_ipsilateral));
    ...
    assert(IsEqual(output_a[1], output_contralateral));
    ...
    std::vector<Signal> output_b = stream_a->Pull(6);
    assert(IsEqual(output_b[1], output_ipsilateral));
    assert(IsEqual(output_b[0], output_contralateral));
    ...
    std::vector<Signal> output_e = stream_b->Pull(6);
    
    assert(IsEqual(output_e[0], output_e[1]));
    ...
    assert(IsEqual(output_e[0], output_frontal));
    
    // Testing reset
    assert(stream_b->IsEmpty());
    mic_b.RecordPlaneWave(1.0, Point(0.0,0.0,-1.0));
    assert(! IsEqual(stream_b->PullLeft(), 0.0));
    assert(! IsEqual(stream_b->PullRight(), 0.0));
    assert(stream_b->IsEmpty());
